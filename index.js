...
function eventPort() {
  ...
  if (newShipChance <= 0.5 && player.cash >= 1000) {
    newShip()
  }
  if (moreGunsChance <= 0.5 && player.cash >= 100) {
    moreGuns()
  }
  if (getRobbed <= 0.15) {
...
function moneylender() {
  ...
  while (true) {
    ...
    if (input === "y") {
      ...
      while (true) {
        let input = prompt("How much do you wish to borrow? You can borrow up to " + player.cash + ". ")
        let inputAmount = parseInt(input)
        if (inputAmount > player.cash) {
